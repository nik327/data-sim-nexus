...
import {
  ...
  Loader2, ChevronRight, Circle, Leaf, AlertTriangle,
} from "lucide-react";
import { hubTasksBySector, sectorLabels } from "@/lib/sectorContent";
import { crisisBySector } from "@/lib/crisisData";
import { useCareerStats } from "@/hooks/useCareerStats";
import CrisisOverlay from "@/components/crisis/CrisisOverlay";
import ProfessionalStanding from "@/components/crisis/ProfessionalStanding";
...
export default function WorkHub() {
  ...
  const { stats, addCrisisWin, addCrisisLoss } = useCareerStats();

  const tasks = hubTasksBySector[sector];
  const scenario = crisisBySector[sector];
  ...
  // Random crisis trigger (once per session, after 60-180s)
  useEffect(() => {
  ...
  const handleCrisisResolved = useCallback((success: boolean) => {
    if (success) addCrisisWin();
    else addCrisisLoss();
  ...
  return (
    <>
      {crisisActive && (
        <CrisisOverlay scenario={scenario} sector={sector} onResolved={handleCrisisResolved} />
      )}
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full bg-card rounded-3xl p-4 shadow-lg shadow-accent/5 border border-accent/20 hover:border-accent/40 transition-all duration-300 group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="flex items-center gap-3">
                  <AlertTriangle className="h-5 w-5 text-accent group-hover:animate-pulse" strokeWidth={1.5} />
                  <div className="text-left">
                    <p className="text-xs font-mono uppercase tracking-wider text-accent font-semibold">System Alert</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">Trigger Crisis Drill</p>
                  </div>
                </div>
              </button>

              {/* Professional Standing */}
              <ProfessionalStanding stats={stats} />
